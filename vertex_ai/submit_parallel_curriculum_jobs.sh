#!/bin/bash
# Submit 5 parallel curriculum generation jobs to Vertex AI
...
# Number of parallel jobs (limited by CPU quota: 32 CPUs / 8 per job = 4 max)
NUM_JOBS=4
CYCLES_PER_JOB=500
...
for i in $(seq 1 $NUM_JOBS); do
    ...
    gcloud ai custom-jobs create \
      ...
      --args="--cycles=${CYCLES_PER_JOB},--bucket=${BUCKET_NAME},--provider=vertexai:gemini-2.5-flash,--project-id=${PROJECT_ID},--location=${REGION}" \
      --project=${PROJECT_ID} &
